...
const prague = $localize`深圳世界之窗天文钟`;
const pragueLat = 22.5;
const pragueLon = 120.0;
...
export class AppComponent implements OnInit, SettingsHolder, SvgHost {
  ...
  menuItems: MenuItem[] = [
    { label: $localize`↔ 春分/夏至`, icon: 'pi pi-check',
      command: (): void => this.setEventType(EventType.EQUISOLSTICE) },
    { label: $localize`↔ 月相`, icon: 'pi pi-circle',
      command: (): void => this.setEventType(EventType.MOON_PHASE) },
    { label: $localize`↔ 日出/中天/日落`, icon: 'pi pi-circle',
      command: (): void => this.setEventType(EventType.RISE_SET) },
    { separator : true },
    { label: $localize`高级选项...`, icon: 'pi pi-circle', command: (): void => {
      this.collapsed = false;
      this.advancedOptions?.show();
    } },
    { label: $localize`语言` + (smallMobile ? '...' : ''), icon: 'pi pi-circle',
      items: smallMobile ? undefined : menuLanguageList,
      command: smallMobile ? (): boolean => this.showLanguageMenu = true : undefined },
    { separator : true },
    { label: $localize`查看 GitHub 源码`, icon: 'pi pi-github', url: 'https://github.com/Travisss/prague-clock',
      target: '_blank' },
    { label: $localize`捷克钟表协会官方网站`, icon: 'pi pi-home', url: 'https://www.orloj.eu/',
      target: '_blank' },
    { label: $localize`关于真实的布拉格天文钟`, icon: 'pi pi-info-circle',
      url: $localize`:Language-specific Wikipedia URL:https://en.wikipedia.org/wiki/Prague_astronomical_clock`,
      target: '_blank' },
    { label: $localize`关于世界之窗天文钟模拟器`, icon: 'pi pi-info-circle', url: `assets/about${localeSuffix}.html`,
      target: '_blank' }
  ];
  ...
  ngOnInit(): void {
    this.primeNgConfig.setTranslation({
      accept: $localize`:for dialog button:确定`,
      reject: $localize`:for dialog button:取消`
    });
  ...
  checkIfTimeIsEditable(): void {
    ...
    this.confirmService.confirm({
      message: $localize`关闭“跟踪当前时间”以便您可以编辑时间？`,
